/**
 * mockDb.ts — Client-side localStorage database with full backend parity.
 * Implements: password hashing (Web Crypto), OTP system, RBAC checks,
 * login audit log, notifications, sessions, and all data relationships.
 */
...
// ─── Web Crypto Password Hashing (mirrors bcrypt behavior) ────────
export async function hashPassword(plain: string): Promise<string> {
  const enc = new TextEncoder();
  const data = enc.encode(plain + 'sc_salt_bbdu_2026');
...
// ─── Fake JWT (mirrors server JWTs) ──────────────────────────────
const SECRET = 'sc_mock_jwt_secret_bbdu';
export function signToken(payload: object, expiresInMin = 15): string {
  const exp = Date.now() + expiresInMin * 60 * 1000;
  return btoa(JSON.stringify({ ...payload, exp, iss: 'SmartCampus-Mock', secret: SECRET }));
}
export function verifyToken(token: string): any | null {
  try {
    const p = JSON.parse(atob(token));
    if (p.secret !== SECRET) return null;
...
// ─── Notification helper ──────────────────────────────────────────
export function pushNotification(db: any, userId: string, title: string, body: string, type = 'info', link?: string) {
  db.notifications = db.notifications || [];
  db.notifications.push({ id: uid(), userId, title, body, type, link: link ?? null, isRead: false, createdAt: new Date().toISOString() });
}
...
async function createSeed() {
  ...
  // Pre-hash passwords
  const [adminHash, facHash, stuHash] = await Promise.all([
  ...
  return {
    users: [
      { id: adminId, email: 'admin@bbdu.ac.in',      passwordHash: adminHash, fullName: 'Dr. Rajesh Kumar',  role: 'admin',   isVerified: true, createdAt: now, lastLoginAt: null, failedLoginCount: 0, lockedUntil: null },
      { id: facId1,  email: 'dr.sharma@bbdu.ac.in',  passwordHash: facHash,   fullName: 'Dr. Priya Sharma',  role: 'faculty', isVerified: true, createdAt: now, lastLoginAt: null, failedLoginCount: 0, lockedUntil: null },
      { id: facId2,  email: 'prof.gupta@bbdu.ac.in', passwordHash: facHash,   fullName: 'Prof. Anand Gupta', role: 'faculty', isVerified: true, createdAt: now, lastLoginAt: null, failedLoginCount: 0, lockedUntil: null },
      { id: stuId1,  email: '22cse001@bbdu.ac.in',   passwordHash: stuHash,   fullName: 'Rahul Verma',       role: 'student', isVerified: true, createdAt: now, lastLoginAt: null, failedLoginCount: 0, lockedUntil: null },
      { id: stuId2,  email: '22cse002@bbdu.ac.in',   passwordHash: stuHash,   fullName: 'Priya Agarwal',     role: 'student', isVerified: true, createdAt: now, lastLoginAt: null, failedLoginCount: 0, lockedUntil: null },
      { id: stuId3,  email: '22it001@bbdu.ac.in',    passwordHash: stuHash,   fullName: 'Amit Singh',        role: 'student', isVerified: true, createdAt: now, lastLoginAt: null, failedLoginCount: 0, lockedUntil: null },
    ],

    profiles: [
      { userId: adminId, idCardNumber: 'SC-ADM-2025-10001', department: 'Administration',        designation: 'Vice Chancellor',              phone: '9876543210', bio: null, avatarBase64: null, rollNumber: null,    employeeId: 'EMP-ADM-001', year: null, section: null, course: null, bloodGroup: 'O+', hostelName: null,            hostelRoom: null,  isWarden: false, isLibrarian: false },
      { userId: facId1,  idCardNumber: 'SC-FAC-2025-10002', department: 'Computer Science',       designation: 'Associate Professor',          phone: '9876543211', bio: 'PhD in ML from IIT Delhi.', avatarBase64: null, rollNumber: null, employeeId: 'EMP-FAC-001', year: null, section: null, course: null, bloodGroup: 'A+', hostelName: null,            hostelRoom: null,  isWarden: false, isLibrarian: false },
      { userId: facId2,  idCardNumber: 'SC-FAC-2025-10003', department: 'Information Technology', designation: 'Assistant Professor',          phone: '9876543212', bio: 'Specializes in Web Tech.', avatarBase64: null, rollNumber: null, employeeId: 'EMP-FAC-002', year: null, section: null, course: null, bloodGroup: 'B+', hostelName: null,            hostelRoom: null,  isWarden: true,  isLibrarian: false },
      { userId: stuId1,  idCardNumber: 'SC-STU-2025-20001', department: 'Computer Science',       designation: null,                           phone: '9123456789', bio: null, avatarBase64: null, rollNumber: '22CSE001', employeeId: null, year: 3, section: 'A', course: 'B.Tech CSE', bloodGroup: 'B+', hostelName: 'Vivekananda Block', hostelRoom: 'A-201', isWarden: false, isLibrarian: false },
      { userId: stuId2,  idCardNumber: 'SC-STU-2025-20002', department: 'Computer Science',       designation: null,                           phone: '9123456790', bio: null, avatarBase64: null, rollNumber: '22CSE002', employeeId: null, year: 3, section: 'A', course: 'B.Tech CSE', bloodGroup: 'O-', hostelName: 'Saraswati Block',   hostelRoom: 'B-105', isWarden: false, isLibrarian: false },
      { userId: stuId3,  idCardNumber: 'SC-STU-2025-20003', department: 'Information Technology', designation: null,                           phone: '9123456791', bio: null, avatarBase64: null, rollNumber: '22IT001',  employeeId: null, year: 3, section: 'A', course: 'B.Tech IT',  bloodGroup: 'A-', hostelName: null,            hostelRoom: null,  isWarden: false, isLibrarian: false },
    ],

    otps: [] as any[],       // { id, email, code, type, expiresAt, usedAt }
    sessions: [] as any[],   // { id, userId, token, ipAddress, userAgent, createdAt, expiresAt }
    loginAudit: [] as any[], // { id, userId, email, ipAddress, userAgent, status, timestamp }
    notifications: [
      { id: uid(), userId: stuId1, title: 'Welcome to Smart Campus!', body: 'Your account is ready. Explore modules from the dashboard.', type: 'info', link: '/dashboard', isRead: false, createdAt: now },
      { id: uid(), userId: stuId1, title: 'Assignment Due Soon', body: 'Assignment 1 — Linear Regression is due in 7 days.', type: 'warning', link: '/classroom', isRead: false, createdAt: new Date(Date.now() - 3600000).toISOString() },
    ] as any[],
    ...
    notices: [
      ...
      { id: uid(), title: 'Annual Tech Fest "TECHNOVA 2026" — Registrations Open', content: 'TECHNOVA 2026, the annual technical festival of BBDU, is scheduled for May 10–12, 2026. Events include Hackathon, Robotics, Paper Presentation, Quiz, and Gaming. Register your teams at the Student Affairs Office or online. Cash prizes worth ₹2,00,000.', category: 'cultural', authorId: facId1, authorName: 'Dr. Priya Sharma', pdfFileName: null, pdfBase64: null, createdAt: new Date(Date.now() - 4 * 86400000).toISOString() },
      { id: uid(), title: 'Hostel Fee Payment — Last Date Extended to April 20', content: 'The last date for hostel fee payment for the Academic Year 2025–26 has been extended to April 20, 2026. Students residing in university hostels are required to pay the fee through the university finance portal or at the accounts office between 10:00 AM and 3:00 PM on working days. Late fee of ₹500 per day after due date.', category: 'hostel', authorId: adminId, authorName: 'Dr. Rajesh Kumar', pdfFileName: null, pdfBase64: null, createdAt: new Date(Date.now() - 6 * 86400000).toISOString() },
    ...
    books: [
      ...
      { id: uid(), title: 'Introduction to Algorithms (CLRS)', author: 'Thomas H. Cormen', isbn: '9780262033848', category: 'Computer Science', publisher: 'MIT Press', publishYear: 2022, totalCopies: 10, availableCopies: 7, shelfLocation: 'CS-C1', description: 'CLRS — the bible of algorithms.', createdAt: now },
    ...
    events: [
      { id: uid(), title: 'TECHNOVA 2026 — Annual Tech Fest', description: 'Three-day technical extravaganza featuring Hackathon (24 hrs), Robotics Challenge, Paper Presentation, AI/ML Workshop, Gaming Tournament, and Startup Pitch. Cash prizes worth ₹2,00,000.', venue: 'Main Auditorium & CS Block', eventDate: new Date(Date.now() + 27 * 86400000).toISOString(), endDate: new Date(Date.now() + 29 * 86400000).toISOString(), category: 'technical', organizerType: 'club', organizerName: 'TECHNOVA Committee', clubName: 'Technical Society BBDU', clubContact: '9876543300', facultyName: 'Dr. Priya Sharma', facultyPhone: '9876543211', registrationLink: null, maxParticipants: 500, tags: 'hackathon,robotics,AI,tech', isPublished: true, createdById: facId1, files: [], createdAt: new Date(Date.now() - 5 * 86400000).toISOString() },
      { id: uid(), title: 'BBDU Sports Day 2026', description: 'Annual Inter-College Sports Tournament. Events: Cricket, Football, Basketball, Badminton, Table Tennis, 100m Sprint, Shot Put, Chess, and Carrom.', venue: 'University Sports Ground & Indoor Stadium', eventDate: new Date(Date.now() + 12 * 86400000).toISOString(), endDate: new Date(Date.now() + 13 * 86400000).toISOString(), category: 'sports', organizerType: 'department', organizerName: 'Sports Department', clubName: null, clubContact: null, facultyName: 'Prof. Anand Gupta', facultyPhone: '9876543212', registrationLink: null, maxParticipants: 800, tags: 'cricket,football,sports,tournament', isPublished: true, createdById: facId2, files: [], createdAt: new Date(Date.now() - 3 * 86400000).toISOString() },
      { id: uid(), title: 'Career Guidance Workshop — Campus Placements 2026', description: 'Exclusive workshop for final-year students covering Resume Building, LinkedIn Optimization, Technical Interview Preparation, HR Round Tips, and GD strategies. Guest speakers from TCS, Infosys, and Wipro.', venue: 'Seminar Hall, Admin Block', eventDate: new Date(Date.now() + 5 * 86400000).toISOString(), endDate: null, category: 'academic', organizerType: 'department', organizerName: 'Training & Placement Cell', clubName: null, clubContact: null, facultyName: 'Dr. Rajesh Kumar', facultyPhone: '9876543210', registrationLink: null, maxParticipants: 200, tags: 'placements,career,TCS,Infosys', isPublished: true, createdById: adminId, files: [], createdAt: new Date(Date.now() - 2 * 86400000).toISOString() },
      { id: uid(), title: 'Cultural Fest "TARANG 2026"', description: 'Celebrate art and culture at TARANG 2026! Events include Classical Dance, Western Dance, Solo Singing, Band Performance, Street Play, Photography, and Fashion Show.', venue: 'Open Air Theatre, BBDU Campus', eventDate: new Date(Date.now() + 20 * 86400000).toISOString(), endDate: new Date(Date.now() + 21 * 86400000).toISOString(), category: 'cultural', organizerType: 'club', organizerName: 'Cultural Committee', clubName: 'BBDU Cultural Club', clubContact: '9876543301', facultyName: 'Prof. Anand Gupta', facultyPhone: '9876543212', registrationLink: null, maxParticipants: 600, tags: 'dance,music,culture,fest', isPublished: true, createdById: facId2, files: [], createdAt: now },
    ],
    ...
    communities: [
      { id: uid(), name: 'BBDU Technical Society',  description: 'The official technical club promoting coding, robotics, AI/ML, and innovation. Hosts TECHNOVA every year.',                   type: 'technical',       logoBase64: null, isRecruitmentActive: true,  formUrl: null, memberCount: 245, foundedYear: 2015, tags: 'coding,robotics,AI,hackathon', createdAt: now },
      { id: uid(), name: 'Cultural Club BBDU',       description: 'Celebrates art, music, dance, drama, and cultural diversity. Organizes TARANG — the annual cultural fest.',                  type: 'cultural',        logoBase64: null, isRecruitmentActive: true,  formUrl: null, memberCount: 312, foundedYear: 2013, tags: 'dance,music,theatre,art',      createdAt: now },
      { id: uid(), name: 'NSS Unit BBDU',            description: 'National Service Scheme unit focused on social service, blood donation camps, tree plantation drives, and rural development.', type: 'social',          logoBase64: null, isRecruitmentActive: false, formUrl: null, memberCount: 180, foundedYear: 2012, tags: 'NSS,social service,volunteer',  createdAt: now },
      { id: uid(), name: 'Sports Club',              description: 'Promotes physical fitness and sportsmanship. Organizes intra and inter-college tournaments.',                                  type: 'sports',          logoBase64: null, isRecruitmentActive: true,  formUrl: null, memberCount: 289, foundedYear: 2014, tags: 'cricket,football,badminton',   createdAt: now },
      { id: uid(), name: 'E-Cell BBDU',              description: 'Entrepreneurship Cell fostering startups and innovation. Organizes pitch competitions, startup weekends, and mentorship.',    type: 'entrepreneurship', logoBase64: null, isRecruitmentActive: true,  formUrl: null, memberCount: 134, foundedYear: 2018, tags: 'startup,entrepreneurship',     createdAt: now },
    ],
    ...
    hostelRooms: [
      ...
      { id: 'rm-006', blockId: 'blk-003', roomNumber: 'C-101', floor: 1, capacity: 2, type: 'double', amenities: 'WiFi, Common Bathroom, Study Table',                   isAvailable: true  },
      { id: 'rm-007', blockId: 'blk-003', roomNumber: 'C-201', floor: 2, capacity: 1, type: 'single', amenities: 'WiFi, Attached Bathroom, Study Table, AC, Cupboard',   isAvailable: true  },
    ],
    roomAllocations: [
    ...
    messMenu: [
      ...
      { id: uid(), blockId: 'COMMON', day: 'Friday',    meal: 'breakfast', items: 'Bread Butter, Boiled Egg, Tea/Coffee' },
    ...
    classroomNotes: [
      { id: uid(), classroomId: 'cls-001', uploaderId: facId1, uploaderName: 'Dr. Priya Sharma', title: 'Unit 1 — Introduction to Machine Learning', description: 'Covers supervised and unsupervised learning, linear regression, and decision trees.', fileName: 'ML_Unit1_Notes.pdf', fileBase64: null, fileType: 'application/pdf', createdAt: new Date(Date.now() - 5 * 86400000).toISOString() },
    ...
    classroomMessages: [
      { id: uid(), classroomId: 'cls-001', senderId: facId1, senderName: 'Dr. Priya Sharma', content: 'Dear students, your Unit Test 2 is scheduled for April 20. Syllabus includes Units 3 and 4.', sentAt: new Date(Date.now() - 1 * 86400000).toISOString() },
      { id: uid(), classroomId: 'cls-001', senderId: facId1, senderName: 'Dr. Priya Sharma', content: 'Unit 2 notes have been uploaded. Please go through them before Thursday class.', sentAt: new Date(Date.now() - 2 * 86400000).toISOString() },
    ],

    assignments: [
      { id: 'asgn-001', classroomId: 'cls-001', title: 'Assignment 1 — Linear Regression Implementation', description: 'Implement Linear Regression using Python (NumPy/Pandas). Dataset will be provided. Submit Jupyter Notebook (.ipynb).', pdfBase64: null, pdfFileName: null, dueDate: new Date(Date.now() + 7 * 86400000).toISOString(), createdById: facId1, createdByName: 'Dr. Priya Sharma', createdAt: new Date(Date.now() - 3 * 86400000).toISOString() },
    ],
    assignmentSubmissions: [] as any[],

    lostFound: [
      { id: uid(), type: 'lost',  title: 'Blue Jansport Backpack',                description: 'Lost near the canteen on April 10. Contains notes, calculator, and a water bottle. Name written inside: Rahul Verma.', category: 'bag',       location: 'University Canteen Area',        reporterId: stuId1, reporterName: 'Rahul Verma',      status: 'open',   images: [], createdAt: new Date(Date.now() - 3 * 86400000).toISOString() },
    ...
    mapPins: [
      ...
      { id: uid(), title: 'Admin Block',         description: 'Administration offices, VC secretariat, Accounts',           lat: 26.8738, lng: 80.9922, category: 'admin',    createdById: adminId },
      { id: uid(), title: 'Vivekananda Hostel',  description: 'Boys hostel — Vivekananda Block (5 floors)',                  lat: 26.8760, lng: 80.9918, category: 'hostel',   createdById: adminId },
      { id: uid(), title: 'Saraswati Hostel',    description: 'Girls hostel — Saraswati Block (4 floors)',                   lat: 26.8736, lng: 80.9940, category: 'hostel',   createdById: adminId },
    ],
  };
}
...
export function getDb(): any {
  ...
  // Will be replaced by async initDb; return empty shell if not ready
  return null;
}

export function saveDb(db: any): void {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(db)); }
  catch (e) { console.warn('[MockDB] Save failed (storage full?):', e); }
}
...
// ─── Helper: get db, auto-initialise synchronously if somehow missing ─
export function db(): any {
  const d = getDb();
  if (!d) throw new Error('MockDB not initialised — call await initDb() first');
  return d;
}
