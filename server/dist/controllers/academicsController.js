...
exports.getMyFees = getMyFees;
exports.markFeePayment = markFeePayment;
exports.getFeeSummary = getFeeSummary;
exports.uploadFeeReceipt = uploadFeeReceipt;
exports.reviewFeeReceipt = reviewFeeReceipt;
exports.getPendingReceipts = getPendingReceipts;
exports.getCalendarEvents = getCalendarEvents;
exports.createCalendarEvent = createCalendarEvent;
exports.deleteCalendarEvent = deleteCalendarEvent;
const prisma_1 = require("../utils/prisma");
const academics_types_1 = require("../types/academics.types");
// ═══════════════════════════════════════════════════════════════
//  SUBJECTS
// ═══════════════════════════════════════════════════════════════
...
async function createSubject(req, res) {
    try {
        const parsed = academics_types_1.createSubjectSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten().fieldErrors });
            return;
        }
        const subject = await prisma_1.prisma.subject.create({ data: parsed.data });
...
function calculateGrade(percentage) {
    if (percentage >= 90)
        return { grade: 'O', result: 'pass' };
    if (percentage >= 80)
        return { grade: 'A+', result: 'pass' };
    if (percentage >= 70)
        return { grade: 'A', result: 'pass' };
    if (percentage >= 60)
        return { grade: 'B+', result: 'pass' };
    if (percentage >= 50)
        return { grade: 'B', result: 'pass' };
    if (percentage >= 40)
        return { grade: 'C', result: 'pass' };
    if (percentage >= 33)
        return { grade: 'D', result: 'pass' };
...
async function getMyMarks(req, res) {
    try {
        ...
        const marks = await prisma_1.prisma.subjectMark.findMany({
...
async function uploadMark(req, res) {
    try {
        const parsed = academics_types_1.uploadMarkSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten().fieldErrors });
            return;
        }
        ...
        const mark = await prisma_1.prisma.subjectMark.upsert({
...
async function bulkUploadMarks(req, res) {
    try {
        const parsed = academics_types_1.bulkUploadMarksSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten().fieldErrors });
            return;
        }
        ...
        for (const entry of parsed.data.marks) {
            const subject = await prisma_1.prisma.subject.findUnique({ where: { id: entry.subjectId } });
            if (!subject)
                continue;
...
async function createFeeStructure(req, res) {
    try {
        const parsed = academics_types_1.createFeeStructureSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten().fieldErrors });
            return;
        }
        ...
        const structure = await prisma_1.prisma.feeStructure.create({ data });
...
async function getMyFees(req, res) {
    try {
        ...
        const allStructures = await prisma_1.prisma.feeStructure.findMany({ orderBy: [{ year: 'desc' }, { semester: 'asc' }] });
...
async function markFeePayment(req, res) {
    try {
        const parsed = academics_types_1.markFeePaymentSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten().fieldErrors });
            return;
        }
        const { studentId, feeStructureId, amountPaid, transactionRef, remarks } = parsed.data;
        const payment = await prisma_1.prisma.feePayment.upsert({
...
async function uploadFeeReceipt(req, res) {
    try {
        const userId = req.user.userId;
...
async function reviewFeeReceipt(req, res) {
    try {
        ...
        const data = { receiptStatus: action, approvedById: req.user.userId, approvedAt: new Date() };
        if (action === 'approved') {
            data.status = 'paid';
            data.paidAt = new Date();
        }
        const payment = await prisma_1.prisma.feePayment.update({ where: { id: paymentId }, data });
...
async function getPendingReceipts(req, res) {
    try {
        ...
        const pending = await prisma_1.prisma.feePayment.findMany({
...
async function getCalendarEvents(req, res) {
    try {
        const events = await prisma_1.prisma.academicEvent.findMany({
            orderBy: { date: 'asc' },
...
async function createCalendarEvent(req, res) {
    try {
        const parsed = academics_types_1.createEventSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten().fieldErrors });
            return;
        }
        const data = { ...parsed.data, createdBy: req.user.userId };
        data.date = new Date(data.date);
        if (data.endDate)
            data.endDate = new Date(data.endDate);
        const event = await prisma_1.prisma.academicEvent.create({ data });
...
async function deleteCalendarEvent(req, res) {
    try {
        ...
        await prisma_1.prisma.academicEvent.delete({ where: { id } });
...
//# sourceMappingURL=academicsController.js.map