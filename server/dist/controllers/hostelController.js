...
exports.fileComplaint = fileComplaint;
exports.updateComplaint = updateComplaint;
exports.getFees = getFees;
exports.createFee = createFee;
exports.markFeePaid = markFeePaid;
exports.getHostelStats = getHostelStats;
exports.searchStudents = searchStudents;
exports.toggleWarden = toggleWarden;
exports.getAllAllocations = getAllAllocations;
const prisma_1 = require("../utils/prisma");
// ─── Helper: check warden/admin access ──────────────────────────
async function isHostelStaff(userId, role) {
    if (role === 'admin')
        return true;
    if (role !== 'faculty')
        return false;
    const profile = await prisma_1.prisma.profile.findUnique({ where: { userId }, select: { isWarden: true } });
    return !!profile?.isWarden;
}
// ─── GET /blocks — All hostel blocks ────────────────────────────
async function getBlocks(req, res) {
    try {
        const blocks = await prisma_1.prisma.hostelBlock.findMany({
            where: { isActive: true },
            include: {
...
async function createBlock(req, res) {
    try {
        ...
        if (!name || !type) {
            res.status(400).json({ success: false, message: 'name and type required.' });
            return;
        }
        const block = await prisma_1.prisma.hostelBlock.create({ data: { name, type, totalFloors: totalFloors ?? 4, address, wardenName, wardenPhone } });
        res.status(201).json({ success: true, data: block });
    }
    catch (err) {
        if (err.code === 'P2002') {
            res.status(409).json({ success: false, message: 'Block name already exists.' });
            return;
        }
...
async function getRooms(req, res) {
    try {
        const { blockId, available } = req.query;
        const where = {};
        if (blockId)
            where.blockId = blockId;
        if (available === 'true')
            where.isAvailable = true;
        const rooms = await prisma_1.prisma.hostelRoom.findMany({
            where,
            include: {
...
async function createRoom(req, res) {
    try {
        if (!(await isHostelStaff(req.user.userId, req.user.role))) {
        ...
        if (!blockId || !roomNumber) {
            res.status(400).json({ success: false, message: 'blockId and roomNumber required.' });
            return;
        }
        const room = await prisma_1.prisma.hostelRoom.create({
        ...
        if (err.code === 'P2002') {
            res.status(409).json({ success: false, message: 'Room number already exists in this block.' });
            return;
        }
...
async function getMyRoom(req, res) {
    try {
        ...
        const allocation = await prisma_1.prisma.roomAllocation.findFirst({
        ...
        const fees = await prisma_1.prisma.hostelFee.findMany({
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch room info.' });
    }
}
...
async function allocateRoom(req, res) {
    try {
        ...
        if (!studentId || !roomId || !academicYear) {
            res.status(400).json({ success: false, message: 'studentId, roomId, academicYear required.' });
            return;
        }
        // Check room capacity
        const room = await prisma_1.prisma.hostelRoom.findUnique({
            where: { id: roomId },
        ...
        if (room.allocations.length >= room.capacity) {
            res.status(409).json({ success: false, message: `Room is full (${room.capacity}/${room.capacity}).` });
            return;
        }
        // Vacate existing allocation
        await prisma_1.prisma.roomAllocation.updateMany({ where: { studentId, status: 'active' }, data: { status: 'vacated', checkOutDate: new Date() } });
        const allocation = await prisma_1.prisma.roomAllocation.create({
        ...
        await prisma_1.prisma.hostelRoom.update({ where: { id: roomId }, data: { isAvailable: newCount < room.capacity } });
...
async function vacateRoom(req, res) {
    try {
        if (!(await isHostelStaff(req.user.userId, req.user.role))) {
            res.status(403).json({ success: false, message: 'Staff only.' });
            return;
        }
        const allocation = await prisma_1.prisma.roomAllocation.update({
            where: { id: String(req.params.id) },
            data: { status: 'vacated', checkOutDate: new Date() },
            include: { room: true },
        });
        await prisma_1.prisma.hostelRoom.update({ where: { id: allocation.roomId }, data: { isAvailable: true } });
...
async function getGatePasses(req, res) {
    try {
        ...
        if (status && status !== 'all')
            where.status = status;
        const passes = await prisma_1.prisma.gatePass.findMany({
            where,
...
async function applyGatePass(req, res) {
    try {
        ...
        if (!purpose || !destination || !outDateTime || !expectedReturn) {
            res.status(400).json({ success: false, message: 'purpose, destination, outDateTime, expectedReturn required.' });
            return;
        }
        const pass = await prisma_1.prisma.gatePass.create({
        ...
        res.status(500).json({ success: false, message: 'Failed to apply for gate pass.' });
    }
}
...
async function updateGatePass(req, res) {
    try {
        ...
        if (!isStaff) {
            if (status !== 'returned') {
                res.status(403).json({ success: false, message: 'You can only mark your pass as returned.' });
                return;
            }
            const updated = await prisma_1.prisma.gatePass.update({
        ...
        const updated = await prisma_1.prisma.gatePass.update({ where: { id }, data });
...
async function getMessMenu(req, res) {
    try {
        const menus = await prisma_1.prisma.messMenu.findMany({ orderBy: [{ day: 'asc' }, { meal: 'asc' }] });
        res.json({ success: true, data: menus });
    }
    catch (err) {
        res.status(500).json({ success: false, message: 'Failed to fetch mess menu.' });
    }
}
...
async function updateMessMenu(req, res) {
    try {
        ...
        if (!day || !meal || !items) {
            res.status(400).json({ success: false, message: 'day, meal, items required.' });
            return;
        }
        const menu = await prisma_1.prisma.messMenu.upsert({
        ...
        console.error(err);
        res.status(500).json({ success: false, message: 'Failed to update mess menu.' });
    }
}
...
async function getComplaints(req, res) {
    try {
        ...
        if (type && type !== 'all')
            where.type = type;
        const complaints = await prisma_1.prisma.hostelComplaint.findMany({
            where,
...
async function fileComplaint(req, res) {
    try {
        ...
        if (!type || !title || !description) {
            res.status(400).json({ success: false, message: 'type, title, description required.' });
            return;
        }
        const complaint = await prisma_1.prisma.hostelComplaint.create({
...
async function updateComplaint(req, res) {
    try {
        const { userId, role } = req.user;
        if (!(await isHostelStaff(userId, role))) {
            res.status(403).json({ success: false, message: 'Staff only.' });
            return;
        }
        ...
        const updated = await prisma_1.prisma.hostelComplaint.update({ where: { id }, data });
...
async function getFees(req, res) {
    try {
        const { userId, role } = req.user;
        const isStaff = await isHostelStaff(userId, role);
        const where = isStaff ? {} : { studentId: userId };
        const { status } = req.query;
        if (status && status !== 'all')
            where.status = status;
        const fees = await prisma_1.prisma.hostelFee.findMany({
...
async function createFee(req, res) {
    try {
        ...
        if (!studentId || !amount || !period || !dueDate) {
            res.status(400).json({ success: false, message: 'studentId, amount, period, dueDate required.' });
            return;
        }
        const fee = await prisma_1.prisma.hostelFee.create({
...
async function markFeePaid(req, res) {
    try {
        if (!(await isHostelStaff(req.user.userId, req.user.role))) {
            res.status(403).json({ success: false, message: 'Staff only.' });
            return;
        ...
        const updated = await prisma_1.prisma.hostelFee.update({
            where: { id: String(req.params.id) },
        ...
        res.status(500).json({ success: false, message: 'Failed to mark fee paid.' });
    }
}
...
async function getHostelStats(req, res) {
    try {
        ...
        const [totalBlocks, totalRooms, activeAllocations, pendingPasses, openComplaints, urgentComplaints, pendingFees] = await Promise.all([
            prisma_1.prisma.hostelBlock.count({ where: { isActive: true } }),
            prisma_1.prisma.hostelRoom.count(),
            prisma_1.prisma.roomAllocation.count({ where: { status: 'active' } }),
            prisma_1.prisma.gatePass.count({ where: { status: 'pending' } }),
            prisma_1.prisma.hostelComplaint.count({ where: { status: { in: ['open', 'in_progress'] } } }),
            prisma_1.prisma.hostelComplaint.count({ where: { status: 'open', priority: 'urgent' } }),
            prisma_1.prisma.hostelFee.count({ where: { status: 'pending' } }),
        ]);
        const feeSum = await prisma_1.prisma.hostelFee.aggregate({ where: { status: 'pending' }, _sum: { amount: true } });
        res.json({
            success: true,
...
async function toggleWarden(req, res) {
    try {
        if (req.user.role !== 'admin') {
            res.status(403).json({ success: false, message: 'Admin only.' });
            return;
        ...
        res.status(500).json({ success: false, message: 'Failed to update warden status.' });
    }
}
...
async function getAllAllocations(req, res) {
    try {
        ...
        const { status } = req.query;
        const allocations = await prisma_1.prisma.roomAllocation.findMany({
