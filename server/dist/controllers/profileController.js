...
const profile_types_1 = require("../types/profile.types");
// ─── Helper: generate campus ID card number ───────────────────────────────
function generateIdCardNumber(role) {
...
async function getOrCreateProfile(userId, role) {
    let profile = await prisma_1.prisma.profile.findUnique({ where: { userId } });
    if (!profile) {
...
async function updateMyProfile(req, res) {
    try {
        const { userId, role } = req.user;
        const parsed = profile_types_1.updateProfileSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({
                success: false,
...
async function listUsers(req, res) {
    try {
        const parsed = profile_types_1.listUsersQuerySchema.safeParse(req.query);
        if (!parsed.success) {
            res.status(400).json({
                success: false,
        ...
        const profileFilter = {};
        if (year)
            profileFilter.year = year;
        if (section)
            profileFilter.section = section;
        if (course)
...
async function updateUserRole(req, res) {
    try {
        ...
        const parsed = profile_types_1.updateUserRoleSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({
                success: false,
        ...
        const user = await prisma_1.prisma.user.findUnique({ where: { id: String(targetId) } });
        if (!user) {
            res.status(404).json({ success: false, message: 'User not found.' });
            return;
        }
...
async function updateUserProfile(req, res) {
    try {
        const targetId = String(req.params.userId);
        const profile = await prisma_1.prisma.profile.findUnique({ where: { userId: targetId } });
        if (!profile) {
        ...
        const data = {};
        if (year !== undefined)
            data.year = year ? Number(year) : null;
        ...
        if (rollNumber !== undefined)
            data.rollNumber = rollNumber || null;
        if (phone !== undefined)
            data.phone = phone || null;
        if (bloodGroup !== undefined)
            data.bloodGroup = bloodGroup || null;
        if (hostelName !== undefined)
            data.hostelName = hostelName || null;
        if (hostelRoom !== undefined)
            data.hostelRoom = hostelRoom || null;
...
async function bulkUpdateProfiles(req, res) {
    try {
        ...
        if (updates.year !== undefined)
            data.year = updates.year ? Number(updates.year) : null;
        if (updates.section !== undefined)
            data.section = updates.section || null;
        if (updates.course !== undefined)
            data.course = updates.course || null;
        if (updates.department !== undefined)
            data.department = updates.department || null;
