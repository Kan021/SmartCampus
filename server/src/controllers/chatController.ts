  ...
  try {
    ...
    const users = await prisma.user.findMany({
      ...
      select: {
        ...
        fullName: true,
        role: true,
        profile: { select: { year: true, section: true, course: true, avatarBase64: true } },
      },
      take: 30,
    });
...
export const getConversations = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const convos = await prisma.chatConversation.findMany({
          ...
          include: {
            user: {
              select: {
                id: true,
                fullName: true,
                profile: { select: { avatarBase64: true, year: true, section: true } },
              },
            },
          },
        },
      ...
      orderBy: { updatedAt: 'desc' }, // most-recently-messaged first
    });
...
export const startConversation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!targetUserId) {
      res.status(400).json({ success: false, message: 'targetUserId required' });
      return;
    }
    ...
    const existing = await prisma.chatConversation.findFirst({
      ...
      include: {
        participants: {
          include: { user: { select: { id: true, fullName: true, profile: { select: { avatarBase64: true } } } } },
        },
      },
    });
...
export const getMessages = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await prisma.chatMessage.updateMany({
      where: { conversationId: convoId, senderId: { not: userId }, isRead: false },
      data: { isRead: true },
    });
...
export const sendMessage = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const otherParticipant = await prisma.chatParticipant.findFirst({
      where: { conversationId: convoId, userId: { not: userId } },
    });
    if (otherParticipant) {
      const block = await prisma.blockedUser.findFirst({
        where: {
    ...
    // Bump conversation's updatedAt so it surfaces in sorted list
...
export const getBlockedUsers = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const blocks = await prisma.blockedUser.findMany({
      ...
      include: {
        blocked: { select: { id: true, fullName: true, profile: { select: { avatarBase64: true } } } },
      },
    });
